...
export async function updateMe(userId: string, data: { name?: string; surname?: string }) {
  const current = await pool.query(`select id, name, surname from public.users where id = $1`, [
    userId,
  ]);
  ...
  const result = await pool.query(
    `update users
        ...
        where id = $3
        returning id, email, name, surname, role, email_confirmed, is_active, created_at`,
    [data.name ?? user.name, data.surname ?? user.surname, userId],
  );
